...
public class ThirstManager {
	...
	private void requestClientUpdate() {
		...
		ServerPlayNetworking.send((ServerPlayerEntity) player, Dew.getThirst_update(), buf);


	}

	private void updateThirst(boolean isThirsty) {
		PacketByteBuf buf = PacketByteBufs.create();
		buf.writeBoolean(isThirsty);
	...
	public void drink (String id){
		int additionalDrink = reg.getNutrients(id);
		if(thirstLevel + additionalDrink > 20) {
			thirstLevel = 20;
			updateThirst(false);
		}else
			increaseThirstLevel(reg.getNutrients(id));
